          ...
          <div class="flex justify-center h-screen">
            <div class="w-full 2xl:w-3/6 pt-5 lg:pt-5 ">
              <form @submit.prevent="Alteruser">
                <div class="cardEvent">
                  <h3 class="text-2xl text-center">Alterar Usuario</h3>

                  <div class="mb-5 mt-3 text-center">
                    <div
                      class="mx-auto w-32 h-32 mb-4 border rounded-full relative bg-gray-100 shadow-inset"
                    >
                      <img
                        id="image"
                        class="object-cover w-full h-32 rounded-full mb-5"
                        :src="image"
                      />
                    </div>
...
export default {
  ...
  methods: {
    Alteruser() {
      User.alteruser(localStorage.getItem("public_id"),this.user)
        .then((response) => {
    ...
    if (localStorage.getItem("public_id")) {
      User.user(localStorage.getItem("public_id"))
        .then((response) => {
          ...
          this.user.name = this.userData["user"]["name"];
          this.user.email = this.userData["user"]["email"];
          this.user.profession = this.userData["user"]["profession"];
          this.user.gender = this.userData["user"]["gender"];
        })
        .catch((e) => {
          this.toast("danger", e.response.data["message"]);
        });
